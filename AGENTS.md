...
- Tool pins live in `mise.toml`, resolved metadata in `mise.lock`, upstream
  ...
  tool (a full `mise lock` also re-resolves the conda packages and strips the
  Zig checksums), keep `lockfile_version = 1` (CI's mise 2026.9.1 cannot read
  version 2), and run `mise run check`. Native Clang builds native tools; the
  WASI SDK Clang stays off `PATH`.
- Keep the Zig pin equal to `ref/capnp-zig/mise.toml`. ziglang.org no longer
  ...
  CONTRIBUTING.md lists the steps. Run `mise run mirror:zig -- lock --write`
  after any `mise lock` that includes zig; `check:zig-lock` in `lint` fails
  until then.
