...
throws from the WASI import, so the guest stops at that call and `cause` names
the limit. Go's wazero filesystem interface defines no `ENOSPC` or `EFBIG`
...
A later change adds `kind: "exit" | "trap" | "limit" | "protocol"` and
`limit?: keyof ResourceLimits` to `CompileError`. Go callers derive the kind
from the fields that exist today: `Limit != ""` is a limit, `ExitCode != 0` is
an exit, and otherwise `Err` is the runtime's trap error or one of the two
contract messages above.
...
| Cancellation        | Worker: `signal` and `timeoutMs`, with termination; direct: none                                    | `context.Context` deadline or cancellation stops the guest, including one sleeping in `poll_oneoff` |
...
| Shutdown            | `dispose()` rejects pending work and terminates the worker                                          | `Close(ctx)` rejects new calls, waits for active ones, and terminates them when `ctx` ends          |
...
| Runtime requirement | Deno 2.9.6 (direct), exactly Deno 2.6.8 (worker), or a browser with standardized exception handling | Go 1.25 or later with the pinned wazero pseudo-version                                              |
