...
| Cancellation        | Both modes: `signal` and `timeoutMs` (default 30 s), enforced by checks injected into every guest, which stop a guest that loops, recurses, or sleeps in `poll_oneoff`. A timeout keeps the worker, and so does an abort where a `SharedArrayBuffer` reaches the worker; an abort without one terminates it. A direct job sees an abort before its next guest stage | `context.Context` deadline or cancellation stops the guest, including one sleeping in `poll_oneoff` |
...
| Shutdown            | `dispose()` rejects pending work and terminates the worker; a running guest stops at once where a `SharedArrayBuffer` reaches the worker, and otherwise when `terminate()` or its own `timeoutMs` stops it                                                                                                                                                          | `Close(ctx)` rejects new calls, waits for active ones, and terminates them when `ctx` ends          |
