...
| Zig source synchronization: the prepared source tree and 36 mirrored fixtures match `generators/zig/sync.json`                 | `check:zig-sync` passes for native commit `0fb8df4`                                                                         | Passing; it runs inside every `build:zig`                                                                                                                                                                 | 2026-09-15    | `scripts/check-zig-sync.ts`, invoked by `scripts/build-zig.sh`                                        |
...
| Stable SDK interface                                                                                                           | A written contract shared by the TypeScript and Go SDKs, frozen public types, and conformance tests across hosts            | Not started. The SDKs differ in options, limits, and error shapes; see the TypeScript-versus-Go table in the [README](../README.md#host-sdks)                                                             | 2026-09-22    | None                                                                                                  |
...
`0fb8df40126ea166f95016963c465b03db22819e` (the `ref/capnp-zig` gitlink and
`sync.json` `nativeCommit`), the accepted Wasm revision `94ba6b2`, the rules,
and one audited cycle. Its last audited scheduled date is 2026-09-09 and its
consecutive count is 1; it has not been updated since commit `b8d8e3f`. This
repository has no scheduled workflow of its own, and the daily follow-up the
