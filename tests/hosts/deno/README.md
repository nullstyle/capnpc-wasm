...
`capnpc-c++`, ...), as the SDKs and the launcher pass it, so diagnostics never
contain the host path. Stdin, stdout, and stderr remain binary; stdin is read to
EOF before execution. The optional single `--dir host::/` loads a trusted
staging directory into guest `/`. Only successful command exits copy created or
modified files back to that directory. Deletions are not exported. Symlinks,
special files, and invalid path components are rejected. Host environment
