...
the packaged launcher pass it, so diagnostics never contain the host path.
Stdin, stdout, and stderr are inherited; stdin is presented to the guest as a
plain stream, so a redirected regular file behaves like a pipe. Host environment
variables and directories are not inherited. Use repeatable `--dir host::guest`
options to grant directories; for example, `--dir build/compiler-root::/`.
Directory grants are writable, so use disposable staging directories when
running generators. This is a test runner, not a host SDK or a publication
mechanism.
...
the host binary, under a directory keyed by the SHA-256 of the binary. An
identical rebuild reuses the cache (Go refreshes the output's modification time,
so the time cannot be the key); a different build, for example after a wazero
bump, starts a fresh directory and removes the other builds' ones on its first
run. Concurrent hosts of one build share the cache safely; `mise run clean`
removes it with the rest of `build/`. The interpreter compiles no machine code
and uses no cache.
