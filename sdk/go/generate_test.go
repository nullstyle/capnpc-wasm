...
func TestGenerate(t *testing.T) {
	...
	compiler, err := capnpwasm.New(t.Context(), modules)
	if err != nil {
		t.Fatal(err)
	}
	...
	generator, err := capnpwasm.New(t.Context(), modules)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("caller ownership and concurrent jobs", func(t *testing.T) {
		...
		got, err := generator.Generate(t.Context(), capnpwasm.GenerationRequest{Request: input, Generators: []string{"rust"}})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("malformed requests preserve guest diagnostics", func(t *testing.T) {
		for _, language := range []string{"cpp", "rust", "go"} {
			for _, input := range [][]byte{{0xff, 0xff, 0xff, 0xff}, compiled.Request[:len(compiled.Request)-1]} {
				...
				if !reflect.DeepEqual(got, capnpwasm.GenerationResult{}) {
	...
	t.Run("later failure discards earlier generator outputs", func(t *testing.T) {
		...
		got, err = generator.Generate(t.Context(), capnpwasm.GenerationRequest{Request: compiled.Request, Generators: []string{"go"}})
	...
	t.Run("cancelled request", func(t *testing.T) {
		...
		cancel()
		got, err := generator.Generate(ctx, capnpwasm.GenerationRequest{Request: compiled.Request, Generators: []string{"rust"}})
		if !errors.Is(err, context.Canceled) || !reflect.DeepEqual(got, capnpwasm.GenerationResult{}) {
	...
	assertGenerateValidation(t, generator, capnpwasm.GenerationRequest{Request: compiled.Request, Generators: []string{"rust"}})
}

func assertGenerateValidation(t *testing.T, c *capnpwasm.Compiler, request capnpwasm.GenerationRequest) {
	t.Helper()
...
func TestGenerateUnavailableModule(t *testing.T) {
	...
	assertGenerateValidation(t, c, capnpwasm.GenerationRequest{Request: []byte{1}, Generators: []string{"rust"}})
}
...
func TestGeneratePreservesSuccessfulDiagnostics(t *testing.T) {
	...
	c, err := capnpwasm.New(t.Context(), capnpwasm.Modules{
	...
	defer c.Close(context.Background())
	got, err := c.Generate(t.Context(), capnpwasm.GenerationRequest{Request: []byte{1}, Generators: []string{"rust"}})
	if err != nil {
		t.Fatal(err)
	}
	want := []capnpwasm.Diagnostic{{Stage: "generate", Language: "rust", Message: "x"}}
...
func TestGenerateCancellationDuringGuestExecution(t *testing.T) {
	...
	defer cancel()
	got, err := c.Generate(ctx, capnpwasm.GenerationRequest{Request: []byte{1}, Generators: []string{"rust"}})
	var failure *capnpwasm.Error
	if !errors.Is(err, context.DeadlineExceeded) || !errors.As(err, &failure) || failure.Stage != "generate" || failure.Language != "rust" || !reflect.DeepEqual(got, capnpwasm.GenerationResult{}) {
		t.Fatalf("cancellation: %+v, %v", got, err)
	}
}
